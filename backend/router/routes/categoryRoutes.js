...
router.get("/", async (req, res) => {
  try {
    const data = await categoryController.getCategoryData();
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const data = await categoryController.getCategoryDataById(id);
...
router.post("/create", async (req, res) => {
  try {
    ...
    const result = await categoryController.createCategoryData(newData);
    res.json(result);
  } catch (error) {
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const result = await categoryController.updateCategoryData(id, updatedData);
    res.json(result);
  } catch (error) {
...
router.delete("/:id", async (req, res) => {
  try {
    const id = req.params.id;
    const result = await categoryController.deleteCategoryData(id);
    res.json(result);
  } catch (error) {
